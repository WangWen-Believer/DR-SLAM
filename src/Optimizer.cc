...
*/
    void Optimizer::BundleAdjustment(const vector<KeyFrame *> &vpKFs, const vector<MapPoint *> &vpMP,
        ...
        for (size_t i = 0; i < vpML.size(); i++) {
            ...
            // 将线特征以起点和终点作为顶点添加到优化器中， 以下形式同mapPoint一样
    ...
    int Optimizer::PoseOptimization(Frame *pFrame, bool bStruct) {
        g2o::SparseOptimizer optimizer;
        g2o::BlockSolver_6_3::LinearSolverType *linearSolver;
        ...
        // 建立关于观测到面的一元边
        {
            ...
            for (int i = 0; i < M; ++i) {
                ...
                // 如果匹配到地图平面进行下面操作，当前无匹配平面，则mvpMapPlanes[i]为空
                if (pMP) {
                    nInitialCorrespondences++;
...
*/
        ...
        for (size_t it = 0; it < 4; it++) {
            ...
            for (size_t i = 0, iend = vpEdgesMono.size(); i < iend; i++) {
                ...
                if (chi2 > chi2Mono[it]) {
                    pFrame->mvbOutlier[idx] = true;// 设置为outlier , level 1 对应为外点,上面的过程中我们设置其为不优化
                    e->setLevel(1);
                    nBad++;
                } else {
                    pFrame->mvbOutlier[idx] = false;
                    vMonoPointInfo[i] = 1.0 / sqrt(chi2);// 设置为inlier, level 0 对应为内点,上面的过程中我们就是要优化这些关系
                    e->setLevel(0);
                }
...
//                cout << "Ver Plane: " << PE / PN << endl; //<< " Max: " << PMax << endl;
    ...
    int Optimizer::PoseOptimization_strict(Frame *pFrame) {
        g2o::SparseOptimizer optimizer;
        g2o::BlockSolver_6_3::LinearSolverType *linearSolver;
        ...
        int nBad = 0;
        // 一共进行四次优化
        cout << "vpEdgesPlane.size() " << vpEdgesPlane.size() << endl;
        if(vpEdgesPlane.size() < 2) return 0;
        for (size_t it = 0; it < 4; it++) {
            cout << "size_t it =" << it<< endl;
            vSE3->setEstimate(Converter::toSE3Quat(pFrame->mTcw));
            // 其实就是初始化优化器,这里的参数0就算是不填写,默认也是0,也就是只对level为0的边进行优化
            optimizer.initializeOptimization(0);
            // 开始优化，优化10次
...
// 敲黑板  这里是重点
    // 利用运动模型估计initial R & t
    // 利用追踪到的点线面 来对T进行优化
    int Optimizer::TranslationOptimization(Planar_SLAM::Frame *pFrame,bool bStruct) {
        g2o::SparseOptimizer optimizer;
        g2o::BlockSolver_6_3::LinearSolverType *linearSolver;
        ...
        // Set Frame vertex
        g2o::VertexSE3Expmap *vSE3 = new g2o::VertexSE3Expmap();
        vSE3->setEstimate(Converter::toSE3Quat(pFrame->mTcw)); // initial estimate
        vSE3->setId(0);
        vSE3->setFixed(false);
        optimizer.addVertex(vSE3);
            ...
            for (int i = 0; i < N; i++) {
                ...
                if (pMP) {
                    ...
                    if (pFrame->mvuRight[i] < 0) {
                        ...
                        // OnlyTranslation without Rotation? Designed by Author?
                    ...
                    } else  // Stereo observation
                    {
                        ...
                        // 边的雅克比矩阵，作者论文里面不是说要固定R，单独求t吗，这里面并没有固定R呀
                        ...
                        e->bf = pFrame->mbf;
                        cv::Mat Xw = pMP->GetWorldPos();
                        //  这里虽然说单纯的乘了一个旋转R 但是在这个边的computeError中 他会调去顶点上的t  即 Xc + t
                        cv::Mat Xc = R_cw * Xw;
        ...
        vector<EdgeLineProjectXYZOnlyTranslation *> vpEdgesLineSp;//start point
        vector<size_t> vnIndexLineEdgeSp;
        vpEdgesLineSp.reserve(NL);
        vnIndexLineEdgeSp.reserve(NL);

        vector<EdgeLineProjectXYZOnlyTranslation *> vpEdgesLineEp;// end point
        vpEdgesLineEp.reserve(NL);
        ...
        //Set Plane vertices
        // 在进行匹配中并为将其跟上一帧进行匹配  仅是将自身检测到的平面投影到世界坐标系下
        // 我的疑问是平面之间是如何关联的
        const int M = pFrame->mnPlaneNum;
...
//        vnIndexEdgePlanePoint = vector<vector<size_t>>(M);
        ...
        double angleInfo = Config::Get<double>("Plane.AngleInfo"); // 角度的信息矩阵
        angleInfo = 3282.8 / (angleInfo * angleInfo);
        double disInfo = Config::Get<double>("Plane.DistanceInfo");
        disInfo = disInfo * disInfo;
        double parInfo = Config::Get<double>("Plane.ParallelInfo");
        ...
        double verInfo = Config::Get<double>("Plane.VerticalInfo");
        verInfo = 3282.8 / (verInfo * verInfo);
        double planeChi = Config::Get<double>("Plane.Chi");        //  鲁棒核
        const float deltaPlane = sqrt(planeChi);
...

        {
            ...
            for (int i = 0; i < M; ++i) {
                ...
                if (pMP) {
                    ...
                    // 这个顶点就是我们待优化的变量
                    e->setVertex(0, dynamic_cast<g2o::OptimizableGraph::Vertex *>(optimizer.vertex(0)));
                    // 我现在有measurement 但是哪里有这个空间中的平面的   这里面就没有什么匹配这一说吗
                    // 在这里确实没有匹配这一步骤    但是在局部地图追踪时有平面匹配
                    // 在进行初始化的时候  我们将第一帧的位姿固定 eye, 同时将他的面投影至世界坐标中，此时我们认为这个面是绝对的，后面的面都得以他为参考
