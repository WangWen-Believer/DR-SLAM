...
// ransac
{
    ...
    vector<int> indexes;
    for(size_t i=0; i<a.size(); ++i)	indexes.push_back(i);
    int iter = 0;
    ...
    for(size_t i=0; i<maxConSet.size();++i) {
    ...
    while(1) {
        ...
        if(conset.size() <= prevConSet.size())
            ...
            ina.clear(); inb.clear();
            for(size_t i=0; i<prevConSet.size();++i) {
...
void costFun_MLEstimateLine3d_compact(double *p, double *error, int m, int n, void *adata)
{
    ...
    for(size_t i=0; i<dptr->pts.size(); ++i) {
        if(i==idx1) {
...
// output: inlier points, line parameters: midpt and direction
{
    ...
    rl.director=(rl.A-rl.B)/sqrt((rl.A-rl.B).dot(rl.A-rl.B));
    rl.mid=(rl.A+rl.B)/2;
//    rl.direct1 = (rl.A-rl.mid)/sqrt((rl.A-rl.mid).dot(rl.A-rl.mid))
//    rl.direct2 = (rl.B-rl.mid)/sqrt((rl.B-rl.mid).dot(rl.B-rl.mid))
...
//	MyTimer t; t.start();
    ...
    //double cost = 0;
#pragma omp  parallel for
    for(size_t i=0; i< dptr->a.size() ; ++i)	{
